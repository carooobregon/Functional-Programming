...
(define (qsort lst)
	(if	(null? lst)
   		null
   		(append (qsort (sm (car lst) (cdr lst))) (list (car lst)) (qsort (lg (car lst) (cdr lst))))
  	)
)
...
(define (get-balance movements)
	(-
		(apply + (map (lambda (x) (caddr x)) (filter (lambda (x) (equal? (car x) "income")) movements)))
		(apply + (map (lambda (x) (caddr x)) (filter (lambda (x) (equal? (car x) "expense")) movements)))
	)	
)
...
(define (toUpperTriangular matrix)
	(toUpperTriangularAux matrix 0)
)

(define (toUpperTriangularAux matrix i)
	(if (null? matrix)
		'()
		(cons (replace (car matrix) i) (toUpperTriangularAux (cdr matrix) (+ i 1)))
	)
)

(define (replace row n)
	(if (= n 0)
		row
		(cons 0 (replace (cdr row) (- n 1)))
	)
)
...
(define (dfs tree)
	(cond 
		...
		(else (append (list (car tree)) (dfs (cdr tree))))
	)
)
